...
import { env, nowIso, correlationId, getDeptWebhook, invokeAgent, logActivity } from './utils/cockpitHelpers';
...
// Helper functions imported from utils/cockpitHelpers
...
export default function EconeuraCockpit({ user, onLogout }: EconeuraCockpitProps) {
  ...
  // const [listening, setListening] = useState(false); // Managed by hook
  ...
  // const recognitionRef removed (allocated in hook)
  ...
  // Voice: TTS + STT via Hook
  const { listening, speak, stopSpeak, toggleListen } = useSpeechRecognition(setChatInput);
